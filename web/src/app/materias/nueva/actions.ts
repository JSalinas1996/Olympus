...
export async function createSubject(formData: FormData) {
  ...
  const { error } = await supabase.from("subjects").insert({ owner_id: user.id, name: parsed.data.name, code: parsed.data.code || null, period: parsed.data.period || null, student_prompt: parsed.data.studentPrompt, professor_prompt: parsed.data.professorPrompt });
