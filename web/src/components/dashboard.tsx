  ...
  if (view === "list") {
    return (
      <article className="group grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-lg md:grid-cols-[minmax(230px,1.4fr)_1fr_1fr_auto] md:items-center">
        ...
        <Link href={`/materias/${subject.id}`} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold transition group-hover:border-slate-900 group-hover:bg-slate-950 group-hover:text-white">Abrir materia</Link>
      </article>
    );
  }

  return (
    <Link href={`/materias/${subject.id}`} className="group flex min-h-72 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-xl">
