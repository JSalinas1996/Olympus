...
type FinalDelivery = { id: string; name: string; mimeType: string; url: string | null; evaluation: string } | null;
...
type NativeComplete = NativeProgress & { fileName: string; mimeType: string; fileBase64: string; evaluation: string; rounds: number };
...
export function NativeCycle(props: Props) {
  ...
  const start = () => {
    ...
    setScore(null);
    setPendingFinal(null);
    send("start-file-cycle", { prompt, professorPrompt: props.professorPrompt, reviewContext: props.reviewContext, format, maxRounds: 3 });
  };
