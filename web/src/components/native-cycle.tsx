...
export function NativeCycle(props: Props) {
  ...
  useEffect(() => {
    ...
    const modelCheck = (event: Event) => { const detail = (event as CustomEvent<NativeProgress>).detail; progress(event); setRunning(false); if (detail.status) setStatus(detail.status); };
    detect();
    ...
    window.addEventListener("olympus-model-check-result", modelCheck);
    return () => {
      ...
      window.removeEventListener("olympus-model-check-result", modelCheck);
    };
  }, [publish]);
  ...
  const blockers = [
    ...
    !props.claudeModel.model && "Falta seleccionar el modelo de Claude.",
    !props.chatgptModel.model && "Falta seleccionar el modelo de ChatGPT.",
